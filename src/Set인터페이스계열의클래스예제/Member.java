...
public class Member extends Object{
	...
	@Override //어노테이션(@)    자식 것이 아니라 부모것이다. / 오버라이드는 상속관계일 때 이다. 재사용 개념 => 다형성 개념 포함
	public boolean equals(Object obj) { //true =1, false = 0 
		if(obj instanceof Member) { // 인스턴스오브 객체의 타입을 비교하는 것
			Member member =(Member) obj;
			return member.name.equals(name) && (member.age == age) //세가지가 다 정답이면 참
					&& member.juminNum.equals(juminNum);
		}else {//set계열은 인덱스 계열이 없다.
